...
function showDetails(x){$('#dialogContent').innerHTML=`<p class="eyebrow">${x.category}</p><h2>${x.name}</h2><p>${x.description}</p><div class="badges">${(x.bodyAreas||x.focus||[]).map(y=>`<span class="badge">${y}</span>`).join('')}</div><h3>Junior</h3><p>${x.junior||'-'}</p><h3>Voksen</h3><p>${x.adult||'-'}</p><button id="dialogAddBtn">Tilføj</button>`;$('#dialogAddBtn').onclick=()=>{builder.addExercise(x.id);$('#exerciseDialog').close()};$('#exerciseDialog').showModal()}
function save(){let w={id:currentId||crypto.randomUUID(),name:$('#workoutName').value,date:$('#workoutDate').value,participants:+$('#participantCount').value,familyMode:$('#familyMode').checked,adultCount:+($('#adultCount').value||0),sections:builder.getData(),music:{name:$('#playlistName').value,url:$('#playlistUrl').value}};currentId=w.id;saveWorkout(w);renderSaved();alert('Træningen er gemt.')}
function renderSaved(){let a=loadSavedWorkouts(),el=$('#savedWorkouts');el.innerHTML=a.length?a.map(w=>`<div class="saved-item"><div><strong>${w.name}</strong><br><small>${w.date||''}${w.familyMode?` · Familie (${w.adultCount||0} voksne)`:''}</small></div><div><button data-open="${w.id}" class="secondary">Åbn</button><button data-delete="${w.id}" class="ghost">Slet</button></div></div>`).join(''):'<div class="empty-state">Ingen gemte træninger endnu.</div>';el.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>{let w=a.find(x=>x.id===b.dataset.open);currentId=w.id;$('#workoutName').value=w.name;$('#workoutDate').value=w.date;$('#participantCount').value=w.participants;$('#familyMode').checked=!!w.familyMode;$('#adultCount').value=w.adultCount||0;$('#adultCountLabel').classList.toggle('hidden',!w.familyMode);$('#familyModeMessage').classList.toggle('hidden',!w.familyMode);builder.loadData(w.sections);$('#playlistName').value=w.music?.name||'';$('#playlistUrl').value=w.music?.url||''});el.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>{deleteWorkout(b.dataset.delete);renderSaved()})}
